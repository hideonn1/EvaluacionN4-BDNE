    ...
    $jsonSchema: {
      ...
      properties: {
        rut: {
          ...
          pattern: "^\\d{7,8}-[\\dkK]$",
          description:
            "RUT del cliente (requerido, formato válido chileno sin puntos)",
        },
        nombre: {
          bsonType: "string",
    ...
    $jsonSchema: {
      ...
      properties: {
        ...
        precio: {
          ...
          description:
            "Precio del producto (requerido, estrictamente mayor a 0)",
        },
        stock: {
          bsonType: ["int", "long"],
          minimum: 0,
    ...
    $jsonSchema: {
      ...
      properties: {
        ...
        rut_cliente: {
          bsonType: "string",
          description:
            "RUT del cliente asociado (requerido, referencia lógica a clientes)",
        },
        items: {
          bsonType: "array",
          minItems: 1,
          description:
            "Arreglo de productos incluidos en el pedido (requerido, al menos 1)",
          items: {
            bsonType: "object",
            required: [
              "producto_id",
...
db.pedidos.createIndex(
  { codigo_pedido: 1 },
  { unique: true, name: "idx_codigo_pedido_unique" },
);
db.pedidos.createIndex({ rut_cliente: 1 }, { name: "idx_rut_cliente" });
db.pedidos.createIndex({ estado: 1 }, { name: "idx_estado" });

print("Creando usuario de aplicación (readWrite)...");
db.createUser({
  user: process.env.APP_USERNAME,
  pwd: process.env.APP_PASSWORD,
  roles: [{ role: "readWrite", db: "comerciotech_db" }],
});

print("Creando usuario de auditoría (read)...");
db.createUser({
  user: process.env.AUDITOR_USERNAME,
  pwd: process.env.AUDITOR_PASSWORD,
  roles: [{ role: "read", db: "comerciotech_db" }],
});

db = db.getSiblingDB("admin");
print("Creando usuario de monitoreo (clusterMonitor)...");
db.createUser({
  user: process.env.MONITOR_USERNAME,
  pwd: process.env.MONITOR_PASSWORD,
